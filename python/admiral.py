...
"""
for fil in files:
  if str(fil).find("rank_")==0 and ".sh" in fil:
    ...
    for lpt in [1,0]:
      for btv in [0,1]:
        ...
        #print "\n%i for %s vars are: %s"%(len(i_vars),li_tag(i)," ".join(i_vars))
          snom="%srank_%s-%ijet-%spTV%s.sh"%(scr_dir,li_tag(i,btv),nj,"lo" if lpt else "hi",tdf_str(tf))
          command="rank_variables %s %s %i %i %i %i %i %i"%(tmva_in_dir,tmva_out_dir,i,nj,lpt,btv,rewrite,tf)
