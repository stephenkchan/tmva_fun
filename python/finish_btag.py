...
"""
for fil in files:
  if "btag_finish_" in fil and ".sh" in fil:
  ...
  for pt in ptv:
    for jet in jets:
      for cut in cuts:
        for ratio in ratios:
          for ct in cts:
            snom="%sbtag_finish_beff%i_MV2c%i_%s_%ijet_%sptv_%s.sh"%(scr_dir,cut,ratio,"cts"if ct else "dis",jet,"lo" if pt else "hi","tF" if tf else "std")
            command="finish_btag %s %i %i %i %i %i %i %i"%(tmva_in_dir,cut,ratio,ct,jet,pt,rewrite,tf)
