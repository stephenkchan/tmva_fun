...
class bdt_ranker : public bdt_base{
 public:
  bdt_ranker(const string&in,const string&out,const vector<TString>&bs,const vector<TString>&vs,int nt=2,int nj=2,int eff_cut=70,int ratio=20,bool cts=true,int lptv=2,bool btv=true,bool wmll=true,bool met=false);
  ...
  vector<double> ranking_front_matter(const string&tag,bool overwrite=false,int trans_DF=0);
  void ranking_back_matter(const vector<TString>&done,const vector<double>&daisy_chain,ofstream&fout,bool overwrite,int trans_DF,bool print=true)const;
 ...
 protected:
  ...
  void rank_loop_one_var(vector<TString>&to_do,vector<TString>&done,vector<double>&daisy_chain,ofstream&fout,const string&tag,bool overwrite,int trans_DF,bool print_header=true,const map<TString,double>&buffer=map<TString,double>())const;
