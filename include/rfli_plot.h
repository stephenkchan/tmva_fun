...
class rfli_plot: public rfli_rank{
 public:
  rfli_plot(const string&in,const string&out,int ptv=0,int nj=2,bool btvs=false);
  vector<TString> all_plots(const TString&pdir,int tDF=0);
  pair<TString,double> single_plots_log_check(int whic,int nj,int ptv,const TString&pdir,int tDF=0,bool split=true);
  pair<TString,double> single_plots(int whic,int nj,int ptv,const TString&pdir,int tDF=0,bool split=true);
