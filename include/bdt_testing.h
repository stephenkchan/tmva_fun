...
class bdt_testing: public bdt_trainer{
 public:
  bdt_testing(const string&in,const string&out,const string&tag,const vector<TString>&vs,bool overwrite=false,int nt=2,int nj=2,int eff_cut=70,int ratio=20,bool cts=true,bool lptv=true,bool wmll=true,bool met=false);
  ...
  pair<TH1D*,TH1D*> make_testing_bdt_dists(int bg=-1,int trans_DF=0)const;
  pair<double,double> print_test_bdts(TCanvas *c,const TString&pdir,int bg=-1,int trans_F=0){return print_bdt_plots(c,make_testing_bdt_dists(bg,trans_F),pdir,"test-"+identifier+sam_tag(bg),sam_label(bg)+" "+region_label(),false);}//bdt_validate((bdt_trainer)*this).val_sig(bg,trans_F).first);}
